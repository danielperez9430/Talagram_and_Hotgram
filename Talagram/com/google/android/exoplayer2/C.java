...
public final class C {
    @Retention(value=RetentionPolicy.SOURCE) @public interface AudioContentType {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface AudioFlags {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface AudioUsage {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface BufferFlags {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface ColorRange {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface ColorSpace {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface ColorTransfer {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface ContentType {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface CryptoMode {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface Encoding {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface NetworkType {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface PcmEncoding {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface SelectionFlags {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface StereoMode {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface StreamType {
    }

    @Retention(value=RetentionPolicy.SOURCE) @public interface VideoScalingMode {
    }
    ...
    public static final int CHANNEL_OUT_7POINT1_SURROUND = 0;
    ...
    static {
        int v0 = Util.SDK_INT < 23 ? 1020 : 6396;
        C.CHANNEL_OUT_7POINT1_SURROUND = v0;
        C.UUID_NIL = new UUID(0, 0);
        C.COMMON_PSSH_UUID = new UUID(1186680826959645954L, -5988876978535335093L);
        C.CLEARKEY_UUID = new UUID(-2129748144642739255L, 8654423357094679310L);
        C.WIDEVINE_UUID = new UUID(-1301668207276963122L, -6645017420763422227L);
        C.PLAYREADY_UUID = new UUID(-7348484286925749626L, -6083546864340672619L);
    }
    ...
    @TargetApi(value=21) public static int generateAudioSessionIdV21(Context arg1) {
        return arg1.getSystemService("audio").generateAudioSessionId();
    }

    public static long msToUs(long arg3) {
        if(arg3 != -9223372036854775807L) {
            if(arg3 == -9223372036854775808L) {
            }
            else {
                arg3 *= 1000;
            }
    ...
    public static long usToMs(long arg3) {
        if(arg3 != -9223372036854775807L) {
            if(arg3 == -9223372036854775808L) {
            }
            else {
